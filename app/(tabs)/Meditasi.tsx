import { PersiapanMeditasi } from '@/pujaData/PersiapanMeditasi';
    ...
    return (
        ...
        <ScrollView className="flex-1" showsVerticalScrollIndicator={false}>
            ...
            <View className="bg-red-800 p-4 rounded-t-xl">
                ...
                <Text className="text-white font-semibold text-base">Video Panduan Meditasi</Text>
                </View>
            </View>
            <View className="p-4 bg-amber-50">
                <WebView
                source={{ uri: 'https://www.youtube.com/live/afAUPQGHWf8?si=M7KEUd_jeougCaBu' }}
                style={{
            ...
            {/* Meditasi Sections */}
            {PersiapanMeditasi.map((section, index) => (
            <View key={index} className="mx-4 mb-4 bg-white rounded-xl shadow-md border-2 border-yellow-300">
                ...
                >
                    ...
                    <Text className={`font-semibold ${isSectionTranslationVisible(index) ? 'text-white' : 'text-yellow-100'}`}>
                        {isSectionTranslationVisible(index) ? 'Sembunyikan Tips' : 'Tampilkan Tips'}
                    </Text>
                    </View>
                    <FontAwesome5
                ...
                <View className="mb-4">
                    ...
                    <Text className="text-yellow-800 font-bold">Langkah-langkah</Text>
                    </View>
                    <View className="bg-white p-4 rounded-lg border-2 border-yellow-200">
                    {section.instructions.map((instruction, lineIndex) => (
                        <View key={lineIndex} className="flex-row mb-2">
                        <Text className="text-yellow-800 mr-2">•</Text>
                        <Text
                ...
                {isSectionTranslationVisible(index) && (
                    <View>
                    <View className="flex-row items-center mb-3">
                        <FontAwesome5 name="lightbulb" size={16} color="#7f1d1d" style={{ marginRight: 8 }} />
                        <Text className="text-red-900 font-bold">Tips</Text>
                    </View>
